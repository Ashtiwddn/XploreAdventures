...
const FacilitiesSection = () => {
  ...
  const descriptionList = [
    "Connect with fellow adventurers who share your passion.",
    "Get expert guidance to enhance your adventure experience.",
    "Explore nearby landmarks and hidden local gems",
  ...
  return (
    <div className="relative z-30 flex flex-col w-full gap-4 lg:gap-6 xl:gap-8 -mt-8 sm:-mt-12 lg:-mt-16 h-fit">
      ...
      <Image
        src="/images/aksen.png"
        className="absolute w-16 -top-10 sm:-top-5 lg:w-20 xl:w-24 lg:-top-10 -left-8 sm:-left-5 lg:-left-20"
        width={500}
        height={500}
        alt="Line Decore"
      />
        ...
        {keys.map((key, index) => (
          <div className="relative">
            <div
              ...
              className="relative z-10 flex flex-col w-40 h-48 mt-1 ml-4 duration-75 bg-white mb-14 lg:h-52 xl:h-64 lg:w-44 xl:w-52 dark:bg-primaryblack hover:shadow-card dark:shadow-slate-700 hover:scale-103 rounded-2xl"
            >
              ...
              <div className="flex flex-col items-center gap-2 py-6 lg:gap-2 xl:gap-4 h-[60%]">
                <h1 className="font-medium text-[10px] lg:text-[11px] xl:text-[13px]">
                  {titleList[index]}
                </h1>
                <h1 className="px-4 lg:px-6 xl:px-8 text-center text-[10px] lg:text-[11px] xl:text-[13px] text-primarygray dark:text-slate-400">
            ...
            <div
              className={`${
                hoverIndex === index ? "scale-100" : "scale-0"
              } flex bottom-10 xl:bottom-8 duration-75 left-0 xl:-left-2 absolute z-0 w-24 h-24 rounded-bl-[25px] md:rounded-bl-[30px] rounded-xl md:rounded-2xl bg-primaryred dark:bg-primaryyellow`}
            ></div>
          </div>
        ))}
      </div>
