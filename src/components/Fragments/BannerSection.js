...
import bannersData from '../../public/banners.json'; // Importing banners data from local JSON file
import AliceCarousel from 'react-alice-carousel';
import "react-alice-carousel/lib/alice-carousel.css";

const BannerSection = () => {
    const [banners, setBanners] = useState(bannersData.data); // Setting banners from local JSON data
    ...
    return (
        <div className='relative flex flex-col w-full gap-10 h-fit text-[10px] lg:text-[11px] xl:text-[13px]'>
            ...
            <div className='flex flex-col items-center sm:items-start px-[20%] sm:px-0 absolute z-20 w-full sm:w-[40%] sm:left-[10%]'>
                <h1 className='overflow-hidden text-lg lg:text-xl xl:text-2xl font-bold tracking-tight left-[20%] font-volkhov whitespace-nowrap sm:animate-typing text-primaryyellow'>
                Adventure Awaits with XploreAdventures!
                </h1>
                ...
                <AliceCarousel infinite autoPlay animationDuration={4000} disableButtonsControls disableDotsControls animationType='fadeout'>
                    {banners.map((banner, index) => (
                        <div key={index} className='relative flex items-center w-full h-52 lg:h-56 xl:h-64 overflow-hidden bg-white text-primaryblack'>
                            { (banner.imageUrl.includes(".jpg") || banner.imageUrl.includes(".png") || banner.imageUrl.includes("images")) ?
