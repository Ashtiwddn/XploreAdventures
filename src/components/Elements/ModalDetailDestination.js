...
const ModalDetailDestination = ({
  showDetailDestination,
  setShowDetailDestination,
  selectedDestination,
  ...
  const handleViewMap = () => {
    setViewMap(true);
    const match = selectedDestination.location_maps.match(
      /<iframe[^>]+src="([^"]+)"/
    );
    if (match && match[1]) {
      setSrcUrl(match[1]);
      ...
      <>
        <div
          className={`absolute z-30 w-full h-full opacity-40 bg-primaryblack dark:bg-primarygray dark:opacity-60 ${
            showDetailDestination === true ? "" : "hidden"
          }`}
        ></div>
        <div
          className={`${
        ...
        >
          ...
          >
            <div className="absolute flex items-center justify-end w-full p-2">
              <button
                onClick={handleCloseDetailDestination}
                className="w-8 h-8 text-xl rounded-lg cursor-default cursor-scale lg:cursor-none hover:text-primaryred dark:hover:text-primaryred"
              >
              ...
              <div className="flex flex-col items-start justify-center gap-4 w-fit h-fit">
                ...
                >
                  <div className="flex items-center gap-2 -mb-2 sm:hidden">
                    <h1 className="text-lg font-semibold capitalize lg:text-xl xl:text-2xl">
                      {selectedDestination.title}
                    </h1>
                    <h1 className="px-1 capitalize rounded-sm bg-slate-100 dark:bg-slate-700">
                  ...
                  <div className="flex flex-col gap-1 w-fit text-primaryblack dark:text-slate-200">
                    ...
                    <div className="flex items-center w-[280px] xs:w-full gap-2 md:gap-[10px] xl:gap-3 px-3 py-1 lg:py-[6px] xl:py-2 mb-1 lg:mb-[6px] xl:mb-2 rounded-lg bg-slate-100 dark:bg-slate-700">
                      <div className="relative flex w-fit">
                        ...
                        <h1 className="relative text-primarygray dark:text-slate-400">
                          {formatNumber(selectedDestination.price)}
                        </h1>
                      </div>
                      <h1 className="text-lg font-semibold lg:text-xl xl:text-2xl text-primaryblue">
                        {formatNumber(selectedDestination.price_discount)}
                      </h1>
                      <h1 className="text-[8px] lg:text-[9px] xl:text-[11px] font-semibold text-white bg-primaryred px-1 rounded-sm">
                        {percent(
                          selectedDestination.price,
                          selectedDestination.price_discount
                        )}
                      </h1>
                    </div>
                    <div className="flex flex-col w-[280px] xs:w-[290px] md:w-[300px] lg:w-[330px] xl:w-[380px] h-[98px] md:h-[143px] lg:h-[155px] xl:h-[160px] gap-2 overflow-y-scroll no-scrollbar">
                      <div className="flex w-auto gap-8 ml-3">
                        <h1 className="w-[30%] text-primarygray dark:text-slate-400">
                          Description
                        </h1>
                        <h1 className="w-[70%]">
                ...
                >
                  <h1 className="pb-2 font-medium text-center capitalize">
                    {selectedDestination.title}
                  </h1>
                  {srcUrl && (
                    <iframe
